...
function transientAnalysis(yValues){
		...
		Array.print(yValues, yValuesMin, yValuesMax, yValuesMean, yValuesStdDev);
		...
		for(j=0;j<maxList.length;j++){
			if(j==maxList.length-1){
				...
				print("rangeSpeedMax:", rangeSpeedMax);
			}
			else{
		...
		print("maxList length is:" +maxList.length);
		if (highFreqBaselineDetection==true){
			for(countPeakRegion=0;countPeakRegion<maxList.length;countPeakRegion++){
				if(countPeakRegion==0){
					...
					print("minValBaseline is:" + minValBaseline);
					print("countPeakRegion is:" + countPeakRegion);
					if (startRange==0 && countPeakRegion==0)
					{
						print ("Skipping drawLine because of invalid values:");
						saveAs("Text", savePath+File.separator+"Error_log_file.txt");
						minValx = 0;
				...
				Plot.setColor("blue");
				// Calculate coords
                //x1 = minValx * samplingTime;
                //y1 = minValBaseline - 10;
                //x2 = minValx * samplingTime;
                //y2 = minValBaseline + 10;

				//if (isNaN(minValx) || isNaN(y1) || isNaN(x2) || isNaN(y2))
				//{
                //    print("Skipping drawLine because of invalid values:");
                //    print("minValx=" + minValx + " samplingTime=" + samplingTime + " minValBaseline=" + minValBaseline);
				//}
				//else
				//{
		...
		else if (drawPeaks==true && lowDown == false){
			setResult("Peak-to-peak time (ms)", k, 0)
			setResult("Baseline value (a.u.)", k, 0);
			setResult("Peak amplitude (a.u.)", k, 0);
			setResult("Contraction amplitude (a.u.)", k, 0);
			}	
